...
public class Tienda {
	...
	private void vuelcaDatos() throws IOException {  // WMC + 1
		...
		for (Vendedor v : lista) {  // WMC + 1    // CCog + 1
			if (v instanceof VendedorEnPracticas) {  // WMC + 1   // CCog + 2
				practicas.add(v);
			} else if (v instanceof VendedorEnPlantilla vp) {  // WMC + 1  // CCog + 1
	...
	private void leeVendedor(Scanner in, TipoVendedor tipo, List<Vendedor> listaVendedores) { // WMC + 1
		...
		switch(tipo) {  // CCog + 1
			case SENIOR: // WMC + 1
				ven = new VendedorSenior(nombreVendedor, idIn, dni);
				break;
			case JUNIOR: // WMC + 1
				ven = new VendedorJunior(nombreVendedor, idIn, dni);
				break;
			case PRACTICAS: // WMC + 1
				ven = new VendedorEnPracticas(nombreVendedor, idIn, dni);
				break;
			default:
				return;
		}
